...
#[tokio::test]
async fn test_get_account_funded() {
    let ctx = setup().await;
    ...
    let result = ctx.backend.get_account(&pubkey).await;
    assert!(result.is_ok());
    let account = result.unwrap();
    assert!(account.is_some());
    let account = account.unwrap();
    assert!(account.lamports >= LAMPORTS_PER_SOL);
}
...
#[tokio::test]
async fn test_get_transaction_existing() {
    let ctx = setup().await;
    let keypair = load_test_keypair();
    ...
    let result = ctx.backend.get_transaction(&sig).await;
    assert!(result.is_ok());
    let parsed_tx = result.unwrap();
    assert!(parsed_tx.is_some());
    let parsed_tx = parsed_tx.unwrap();
    assert_eq!(parsed_tx.signatures.first(), Some(&sig));
    assert!(!parsed_tx.message.account_keys.is_empty());
}
